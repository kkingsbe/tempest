...
fn vcp215_should_have_radials_with_gates() {
    ...
    assert!(
        has_radials,
        "VCP 215 should have at least one sweep with radials"
    );

    // At least one radial should have gates
    let has_gates = volume
        .sweeps
        .iter()
    ...
    assert!(
        has_gates,
        "VCP 215 should have at least one radial with gates"
    );
}
...
fn decode_should_produce_valid_timestamp() {
    ...
    assert!(year >= 2000, "Timestamp year {} should be >= 2000", year);
}
