...
fn save_png(
    ...
    height: u32,
    path: &PathBuf,
) -> Result<(), GoldenImagesError> {
    let img = ImageBuffer::<Rgba<u8>, _>::from_raw(width, height, pixels).ok_or_else(|| {
        GoldenImagesError::Image(image::ImageError::IoError(std::io::Error::new(
            std::io::ErrorKind::InvalidData,
            "Failed to create image from raw pixels",
        )))
    })?;
...
fn generate_golden_images() -> Result<Vec<String>, GoldenImagesError> {
    ...
    let transparent_pixels: Vec<u8> = pixels
        .iter()
        .enumerate()
    ...
    let semi_transparent_pixels: Vec<u8> = pixels
        .iter()
        .enumerate()
        .map(|(i, p)| {
            if i % 4 == 3 {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        ...
        Commands::Verify => {
            ...
            if failed > 0 {
                println!(
                    "Verification FAILED: {}/{} images failed",
                    failed,
                    results.len()
                );
                std::process::exit(1);
                ...
                println!(
                    "Verification PASSED: {}/{} images matched",
                    passed,
                    results.len()
                );
            }
